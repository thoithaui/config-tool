...
$conf = "C:\logstash-8.7.1\config\logstash.conf"
...
& ./nssm.exe install $serviceName $servicePath -f $conf
& ./nssm.exe set $serviceName AppDirectory $AppDirectory
& ./nssm.exe set $serviceName Start SERVICE_AUTO_START
& ./nssm.exe set $serviceName DependOnService "elasticsearch-service-x64"
